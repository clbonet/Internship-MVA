...
def get_edgelist(indptr,indices):
    n = indptr.shape[0]-1
    f = np.empty((indices.shape[0],2))
    
    for i in numba.prange(n):
        row_start = indptr[i]
        row_end = indptr[i+1]
        neighbours = indices[row_start:row_end]
        f[row_start:row_end,0] = i
        f[row_start:row_end,1] = neighbours
